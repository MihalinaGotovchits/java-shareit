...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findAllByBookerIdAndStatePast(Long bookerId, Sort start);
    ...
    @Query("SELECT b FROM Booking AS b " +
            "WHERE b.item.owner.id = ?1 " +
            "AND b.status = ?2")
    List<Booking> findAllByOwnerIdAndStatus(Long ownerId, BookingStatus status, Sort start);
